...
<h1 class="bg-light text-center">
    <c:choose>
        <c:when test="${category != null}">
            Редактирование категории ${category.getName()}
        </c:when>
        <c:otherwise>
...
<div class="container px-5 my-5">
    <form class="row g-3 needs-validation" accept-charset="UTF-8"
          action="${pageContext.request.contextPath}/admin/Categories" method="post">
        <c:choose>
            <c:when test="${category != null}">
        ...
        <div class="form-floating mb-3" <c:if test="${category == null}">hidden</c:if>>
            <input class="form-control" name="categoryId" id="categoryId" type="text"
                   placeholder="Идентификационный номер"
                   value="${category.getId()}" readonly tabindex="-1"/>
            <label for="categoryId">Идентификационный номер</label>
        </div>
        <div class="form-floating mb-3">
            <input class="form-control" name="categoryName" id="categoryName" type="text"
                   ...
                   <c:if test="${category != null}">value="${category.getName()}"</c:if>/>
        ...
        <div class="form-floating mb-3">
            <textarea class="form-control" name="description" id="description"
                      ...
                      pattern="^[А-Яа-яA-Za-z0-9]+(?:\s+[А-Яа-яA-Za-z0-9]+)*$">
<c:if test="${category != null}">${category.getDescription()}</c:if>
            </textarea>
        ...
        <div class="d-grid">
            ...
            <a class="btn btn-danger btn-lg" type="button"
               href="${pageContext.request.contextPath}/admin/Categories">Назад</a>
        </div>
    </form>
</div>
