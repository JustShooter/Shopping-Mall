...
<h1 class="bg-light">Список магазинов</h1>

<div class="container-flex">
    <div class="row">
        <div class="d-flex">
            <table class="table table-hover table-striped bg-light">
                <caption hidden>All shops</caption>
                <thead>
                <tr>
                    <th scope="col">ID</th>
                    <th scope="col">Название</th>
                    <th scope="col">Категория</th>
                    <th scope="col">Владелец магазина</th>
                    <th scope="col">Торговое место</th>
                ...
                <c:forEach var="shop" items="${allShops}">
                    <tr>
                        <th scope="row">${shop.getId()}</th>
                        <td>${shop.getShopName()}</td>
                        <td>${shop.getCategory().getName()}</td>
                        <td>${shop.getShopOwner().getOwnerName()}</td>
                        <td>№ ${shop.getLocation().getShopNumber()}, ${shop.getLocation().getFloor()} этаж</td>
                        <td>
                            <a role="button" class="btn btn-info btn-sm"
                               href="${pageContext.request.contextPath}/admin/Shops?action=edit&shopId=${shop.getId()}">Редактировать</a>
                            <a role="button" class="btn btn-danger btn-sm"
                               href="${pageContext.request.contextPath}/admin/Shops?action=delete&shopId=${shop.getId()}">Удалить</a>
                        </td>
                    </tr>
                </c:forEach>
...
<div class="bg-light d-grid gap-2">
    <a role="button" class="btn btn-success btn-lg"
       href="${pageContext.request.contextPath}/admin/Shops?action=new">Создать</a>
</div>
