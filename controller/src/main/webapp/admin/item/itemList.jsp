...
<h1 class="bg-light">Список товаров</h1>

<div class="container-flex">
    <div class="row">
        <div class="d-flex">
            <table class="table table-hover table-striped bg-light">
                <caption hidden>All Items</caption>
                <thead>
                <tr>
                    <th scope="col">ID</th>
                ...
                <c:forEach var="category" items="${allItems}">
                    <tr>
                        <th scope="row">${category.getId()}</th>
                        <td>${category.getName()}</td>
                        <td>${category.getArticle()}</td>
                        <td>${category.getBarcode()}</td>
                        <td>
                            <a role="button" class="btn btn-info btn-sm"
                               href="${pageContext.request.contextPath}/admin/Items?action=edit&itemId=${category.getId()}">Редактировать</a>
                            <a role="button" class="btn btn-danger btn-sm"
                               href="${pageContext.request.contextPath}/admin/Items?action=delete&itemId=${category.getId()}">Удалить</a>
                        </td>
                    </tr>
                </c:forEach>
...
<div class="bg-light d-grid gap-2">
    <a role="button" class="btn btn-success btn-lg"
       href="${pageContext.request.contextPath}/admin/Items?action=new">Создать</a>
</div>
