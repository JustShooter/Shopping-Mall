...
<h1 class="bg-light">Список владельцев магазинов</h1>

<div class="container-flex">
    <div class="row">
        <div class="d-flex">
            <table class="table table-hover table-striped bg-light">
                <caption hidden>All shopOwners</caption>
                <thead>
                <tr>
                    <th scope="col">ID</th>
                ...
                <c:forEach var="shopOwner" items="${allShopOwners}">
                    <tr>
                        <th scope="row">${shopOwner.getId()}</th>
                        <td>${shopOwner.getOwnerName()}</td>
                        <td>город ${shopOwner.getAddress().getCity()},
                                ${shopOwner.getAddress().getStreetType().getShortName()}
                                ${shopOwner.getAddress().getStreet()},
                            дом № ${shopOwner.getAddress().getBuildingNumber()},
                            офис № ${shopOwner.getAddress().getOfficeNumber()}</td>
                        <td>
                            <a role="button" class="btn btn-info btn-sm"
                               href="${pageContext.request.contextPath}/admin/ShopOwners?action=edit&shopOwnerId=${shopOwner.getId()}">Редактировать</a>
                            <a role="button" class="btn btn-danger btn-sm"
                               href="${pageContext.request.contextPath}/admin/ShopOwners?action=delete&shopOwnerId=${shopOwner.getId()}">Удалить</a>
                        </td>
                    </tr>
                </c:forEach>
...
<div class="bg-light d-grid gap-2">
    <a role="button" class="btn btn-success btn-lg"
       href="${pageContext.request.contextPath}/admin/ShopOwners?action=new">Создать</a>
</div>
