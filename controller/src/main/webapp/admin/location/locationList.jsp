...
<h1 class="bg-light">Список торговых мест</h1>

<div class="container-flex">
    <div class="row">
        <div class="d-flex">
            <table class="table table-hover table-striped bg-light">
                <caption hidden>All locations</caption>
                <thead>
                <tr>
                    <th scope="col">ID</th>
                    <th scope="col">Номер торгового места</th>
                ...
                <c:forEach var="category" items="${allLocations}">
                    <tr>
                        <th scope="row">${category.getId()}</th>
                        <td>№ ${category.getShopNumber()}</td>
                        <td>${category.getFloor()} этаж</td>
                        <td>${category.getDescription()}</td>
                        <td>${category.getShopName()}</td>
                        <td>
                            <a role="button" class="btn btn-info btn-sm"
                               href="${pageContext.request.contextPath}/admin/Locations?action=edit&locationId=${category.getId()}">Редактировать</a>
                            <a role="button" class="btn btn-danger btn-sm"
                               href="${pageContext.request.contextPath}/admin/Locations?action=delete&locationId=${category.getId()}">Удалить</a>
                        </td>
                    </tr>
                </c:forEach>
...
<div class="bg-light d-grid gap-2">
    <a role="button" class="btn btn-success btn-lg"
       href="${pageContext.request.contextPath}/admin/Locations?action=new">Создать</a>
</div>
